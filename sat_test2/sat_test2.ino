...
void loop() {
  ...
  //delay(1200);
  {
    ...
    send(rf95, DATA_BLOCK_START);
    send(rf95, counter);

    send(rf95, bmp_data[0]);
    send(rf95, bmp_data[1]);

    send(rf95, bno_data[0]);
    send(rf95, bno_data[1]);
    send(rf95, bno_data[2]);
    send(rf95, bno_data[3]);
    send(rf95, bno_data[4]);
    send(rf95, bno_data[5]);

    send(rf95, latitude);
    send(rf95, longitude);
    send(rf95, altitude);

    send(rf95, (fan << 0) + (ejected << 1) + (landed << 2));  // Sendet alle Bools in einem Byte

    send(rf95, DATA_BLOCK_END);
...
void BNO(float * dest) {
  ...
  send(rf95, accelerometer.acceleration.x);
  send(rf95, accelerometer.acceleration.y);
  send(rf95, accelerometer.acceleration.z);
  dest[0] = accelerometer.acceleration.x;
  dest[1] = accelerometer.acceleration.y;
  dest[2] = accelerometer.acceleration.z;

  bno.getEvent(&gyroscope, Adafruit_BNO055::VECTOR_EULER);
  send(rf95, gyroscope.gyro.x);
  send(rf95, gyroscope.gyro.y);
  send(rf95, gyroscope.gyro.z);
  dest[3] = gyroscope.gyro.x;
  dest[4] = gyroscope.gyro.y;
  dest[5] = gyroscope.gyro.z;
#endif
}
      ...
      if (isDollarSignFound) {
        ...
        if (x == '\n') {
          ...
          if (isGPGGA) {
            ...
            latitude = gpsdata[0] + gpsdata[1] / 100;
            longitude = gpsdata[2] + gpsdata[3] / 100;
            altitude = gpsdata[4];
          }
        }
      }
...
void send(RH_RF95 rf95, float val) {
#if DEBUG
